import { MenubarTabForm, OpenDialog, ToastMessage } from "@/components";
...
const MenubarTabCard = ({ data, handleSwap, swapValue }: Props) => {
  const { mutateAsync, isPending, isSuccess } =
    _useMutation({ models: "MENUBAR_TAB", methodType: "DELETE", id: data.id });
  const toastMessage = ToastMessage();

  const handleDelete = async () => {
    try {
      await mutateAsync({ path: `${PATH_LIST.MENUBAR_TAB}/${data.id}` });
      ...
      <div className="absolute top-1 right-1 flex items-start gap-2 z-20 scale-75 mr-[-18px]">
        <OpenDialog
          ...
          {{
            ...
            title: "Update",
            desc: "Update Menubar Tab",
            width: "w-[500px]"
          }}>
          <MenubarTabForm type={"UPDATE"} data={data} />
        </OpenDialog>
