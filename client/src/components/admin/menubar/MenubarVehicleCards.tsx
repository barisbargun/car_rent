import { MenubarVehicleCard, ToastMessage } from '@/components';
...
const MenubarVehicleCards = ({ data }: Props) => {
  const { mutateAsync: mutateSwap, isPending: pendingSwap } =
    _useMutation<IMutationBody<ISwapList>>({ models: "MENUBAR_VEHICLE", methodType: "PATCH" });
    ...
    <div className="flex flex-col w-[350px] max-desktop:w-[320px]">
      <div className='flex-center gap-4 py-3 px-1 w-full'>
        <div className='flex-1 line-clamp-3 break-words'>
          <p className="text-sm text-muted-foreground">Menubar title</p>
...
export default MenubarVehicleCards